...
  private lookAheadMatch(
    ...
    const maxA = Math.min(idxA + limit, linesA.length);
    const maxB = Math.min(idxB + limit, linesB.length);

    if (idxA < linesA.length) {
      for (let k = idxB + 1; k < maxB; k++) {
        if (linesA[idxA] === linesB[k]) return { nextA: idxA, nextB: k };
      }
    }
    
    if (idxB < linesB.length) {
      for (let l = idxA + 1; l < maxA; l++) {
        if (linesB[idxB] === linesA[l]) return { nextA: l, nextB: idxB };
      }
    }
    
    return { nextA: -1, nextB: -1 };
  }
...
        let matched = false;
        ...
        if (indexA < linesA.length && indexB < linesB.length) {
            const lookahead = this.lookAheadMatch(linesA, linesB, indexA, indexB, this.MAX_SEARCH);
            
            if (lookahead.nextA === indexA && lookahead.nextB > indexB) { 
                for (let l = indexB; l < lookahead.nextB; l++) {
                    diffs.push({ lineA: '', lineB: linesB[l], status: 'B_ONLY', lineNumA: null, lineNumB: lineNumB++ });
                }
                indexB = lookahead.nextB;
                matched = true;
                continue;

            } else if (lookahead.nextB === indexB && lookahead.nextA > indexA) {
                for (let m = indexA; m < lookahead.nextA; m++) {
                    diffs.push({ lineA: linesA[m], lineB: '', status: 'A_ONLY', lineNumA: lineNumA++, lineNumB: null });
                }
                indexA = lookahead.nextA;
                matched = true;
                continue;
            }
        }
